...
const LABEL = 'block text-xs font-semibold text-gray-600 dark:text-slate-400 mb-1';
const SECTION_BTN = 'flex items-center justify-between w-full px-4 py-3 bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 rounded-xl text-sm font-semibold text-gray-800 dark:text-slate-100 transition';
...
const CVEditorPage = () => {
  ...
  /* Experience helpers */
  const addExp = () => setCvData(prev => ({ ...prev, experiences: [...prev.experiences, { company: '', position: '', role: '', location: '', startDate: '', endDate: '', current: false, description: '', jobDescription: '', skills: [] }] }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col">
      {/* Top bar */}
      <div className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 sticky top-0 z-20 px-4 py-2.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} className="text-base font-bold border-0 focus:outline-none focus:border-b-2 focus:border-primary bg-transparent text-gray-900 dark:text-slate-100" />
        ...
        <div className="flex items-center gap-2">
          ...
          {canUseAI ? (
            ...
            <span className="text-xs text-gray-400 dark:text-slate-500 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-1.5 flex items-center gap-1">🔒 AI (Pro)</span>
          )}
          <button
          ...
          >
            {exporting ? (
              <><svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> Exporting…</>
            ) : (
              <><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg> PDF</>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left panel */}
        <div className="w-[420px] flex-shrink-0 overflow-y-auto bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 p-4 space-y-3">
              ...
              <div className="flex items-start gap-4">
                <div className="relative flex-shrink-0">
                  {cvData.personal_info?.photo ? (() => {
                    ...
                    : <div className="w-16 h-16 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-gray-400 dark:text-slate-500 text-2xl border-2 border-dashed border-gray-300 dark:border-slate-600">👤</div>
                  }
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 dark:text-slate-500 mb-2">JPG, PNG, WebP</p>
                  <input ref={photoRef} type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                  {/* Shape & Size controls */}
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-semibold text-gray-500 dark:text-slate-400 w-10">Shape</span>
                      {[{ key: 'round', label: '⬤ Round' }, { key: 'square', label: '⬛ Square' }].map(({ key, label }) => (
                        <button key={key}
                          ...
                          className={`text-[10px] px-2.5 py-1 rounded-md font-medium transition border ${(cvData.personal_info?.photoShape || 'round') === key
                            ...
                            : 'bg-gray-50 dark:bg-slate-700 text-gray-600 dark:text-slate-400 border-gray-200 dark:border-slate-600 hover:border-primary hover:text-primary'
                            }`}
                        >{label}</button>
                      ))}
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-semibold text-gray-500 dark:text-slate-400 w-10">Size</span>
                      <input
                        type="range"
                      ...
                      />
                      <span className="text-[10px] font-semibold text-gray-600 dark:text-slate-400 w-10 text-right">
              ...
              {cvData.experiences.map((exp, i) => (
                <div key={i} className="p-3 bg-gray-50 dark:bg-slate-700 rounded-xl border border-gray-200 dark:border-slate-600 space-y-2 relative">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="flex items-end pb-2"><label className="flex items-center gap-2 text-xs cursor-pointer text-gray-700 dark:text-slate-300"><input type="checkbox" checked={!!exp.current} onChange={e => updateExp(i, 'current', e.target.checked)} className="accent-primary" /> Currently here</label></div>
                  </div>
                  ...
                  <details className="group">
                    ...
                    <textarea className={`${TEXTAREA} mt-1.5`} rows={3}
            ...
            <div className="pt-1">
              <div className="flex flex-wrap gap-1.5 mb-2">
                {(cvData.interests || []).map((interest, i) => {
                  ...
                  return txt ? (
                    <span key={i} className="flex items-center gap-1 bg-amber-50 dark:bg-amber-950 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-700 rounded-full px-3 py-0.5 text-xs font-medium">
                      {txt}
        ...
        {/* Right: A4 page preview — fills remaining space. Always white for print-safe. */}
